...
| 级别 | 命令 | 用例 | 耗时 | 时机 |
|------|------|------|------|------|
| 快速测试 | `for f in tests/unit/*.test.js; do node "$f" || exit 1; done` | 157 | ~7s | 仅当变更影响整组根级 unit 时 |
| 完整测试 | `node tests/run-all.js` | 218 | ~42s | 跨模块、高风险或任务测试契约明确要求时 |

### 4.2 规则
- 每次代码修改后必须运行该变更等级定义的**最小相关测试**
- 不要使用 `node tests/unit/*.test.js` 作为全量口径（该写法只会执行单个入口文件）
- 全量 unit 必须逐文件执行（见 4.1 快速测试命令）
- 推送前必须完成任务测试契约要求的测试；只有跨模块、高风险或明确要求时运行完整测试
- 纯文档变更不需要运行测试
- 测试失败不得提交
- 新增功能必须添加对应测试
...
| 功能完成 | 变更等级测试 → `git push` | 测试契约要求的最小相关测试 | 批量推送多个 commit |
| 准备发布 | 复用精确 SHA 的 CI 证据 → 关键 smoke → `git tag` / 发布 | 专项发布门禁；不重复无关平台测试 | 打版本标签或部署 |
...
### 7.5 Web Accounting Gate（网页落账语义硬闸门）

**D-076 记账线逐项批准闸门：无已证实错误，不改动记账线。**范围包括原始分段生成、存储、小时/日/周及目标聚合、上传确认、云端物化和记账下发。每项改动必须先向 Product Owner 明示错误证据、具体文件/行为改动、前后差异、少记/多记风险和验证方法，并取得该项明确批准后才能实施。总体计划批准不替代逐项批准；架构统一、代码复用、重构、影子链路或迁移不能成为绕过理由。未确认错误仅允许只读调查和登记，不得先改后报。配额线的改动也不得隐式改变记账线。

`usage_segments_v1` 网页账本是时间配额、Rest 提醒、使用统计和家长判断的金标准。任何改变网页 `ACTIVE` 开始、停止、续账、idle 补偿、窗口焦点、checkpoint 或结算边界的变更，都属于最高风险产品语义变更。

**P0 自动定级规则：**任何已确认或合理怀疑会影响网页/媒体原始账本、上传结算或日/小时/目标物化准确性的问题，一律按 P0 登记，根因未明时不得降级。少记、多记、漏记、重复、重叠、跨日错误、错误域名/分类/mode/quota bucket 归属以及原始账本与物化结果不一致，均属于该规则。问题只发生在特殊网站、单台设备、短时段或小秒数，不构成降级理由；各层统计数值一致也不能排除终端错误被一致继承。任何可能影响网页金标准或配额的缺陷必须阻断相关发布，直至真实浏览器、原始账本对照和固定回归共同证明风险已消除。

**实施前必须：**

1. 通过 Git 历史和当前代码列出旧行为、新行为及被删除或新增的容错。
2. 提供普通输入、强视频、强音频、弱 `tab.audible`、失焦、最小化、后台标签、idle、锁屏和 checkpoint 的前后矩阵。
3. 分别评估少记、多记、配额、统计和历史兼容风险。
4. 获得 Product Owner 对具体网页落账语义的单项明确确认；整份实施计划获批不得替代该确认。
5. 将决策、代码和固定回归测试作为一个独立任务实施，不得混入无关 bugfix、重构、媒体分类或 UI 变更。

**禁止：**

- ❌ 以 bugfix、重构、媒体分类或测试修复名义隐式改变网页落账语义。
- ❌ 只根据当前文档判断产品意图，而不核对规则来源和历史运行行为。
- ❌ 以测试通过证明产品规则正确；测试只证明实现与已确认规则一致。
- ❌ 在缺少真实浏览器与原始网页账本对照时宣称焦点、媒体或 idle 计时准确。
...
- 未明确批准前不跑破坏性系统闸门测试。
- 仅运行与改动文件直接相关的最小测试集合。
- 任务开始时必须记录变更等级、受影响子系统、必要本地测试、必要 CI、发布 smoke 及明确排除项。
- 同一 Git SHA 和相同产物哈希的通过证据必须复用；纯文档或发布证据提交不使既有代码证据失效。
- 若需要更大测试范围，先停止并给出“精确命令 + 风险原因 + 预计耗时”，等待批准；“更完整”不是扩大范围的理由。
