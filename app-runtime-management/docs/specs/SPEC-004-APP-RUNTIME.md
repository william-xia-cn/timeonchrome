...
驱动、runtime、更新/卸载/修复入口、helper、诊断工具、`AppListEntry=none` 组件和证据不足宿主只进入技术记录。名称关键词只能要求审核，不能单独自动隐藏。通用浏览器/PWA 使用可信 AUMID 或 package identity 去重；无法确认的 `Create React App Sample` 类宿主不得进入主目录。

盘点按来源声明完成、带警告或失败。成功来源独立进行缺失对账；单个坏快捷方式不能使其他全部 installed 记录永久保留。目录清理不删除观察或历史账本，不改变 accounting、媒体、分类历史和配额语义。

## ARM-D-013 产品应用投影（当前修复）

产品目录不得把所有历史 `runtimeIdentity` 当作应用。可管理对象限定为已确认产品，或具有 package identity、验证 binary hash/签名等可靠证据且发现角色为主应用的平台实现；同一已确认产品的可信实现合并为一个产品行，分类保存时继续落到现有实现键。只有可管理对象进入五分类目录、分类计数和分类操作。

明确组件/瞬态对象默认隐藏；弱候选及缺少发现证据的历史进程身份进入系统管理的只读技术进程记录。它们保留最近使用、主账本时长和审计原因，但不能直接归类。实现不得用进程显示名黑名单硬编码个例，也不得因名称相似自动合并产品。取得可靠证据或家长确认关联后方可进入产品目录。

验收固定覆盖：安装器 UI 不进入应用目录；短名旧记录不出现分类按钮；确认产品的多个实现只计一个目录对象；helper/updater/组件不增加分类计数；技术记录可审计且不泄露 raw identity；原始 Segment 数量、内容和时长不变。本修复不决定技术进程是否计入主使用账本。

## ARM-D-012 盘点与目录质量修复

安装发现不等于产品确认：同包的多个可见应用不强制合并，隐藏组件默认不铺满目录但可查完整发现，任何运行观察仍正常计时。友好应用名称优先，回退包名须标注。已安装未使用进入可管理目录；保留已使用未归类及处理历史。可信 binary/packageId 支撑安装与旧运行身份的展示关联，不改变历史身份、单应用配额键或分类。未获家长确认的系列关联只建议。

盘点必须显示来源结果、批次进度及完成标记。只有全部预期批次与观察收到并有无失败的结束标记才显示该用户完整；未登录/未盘点用户和旧客户端显示未验证，不能宣称所有账户已完成。失败扫描不能推断卸载。本轮无生产数据清理、部署或自动安装。

## ARM-D-011 应用发现与分类规则（当前阶段）

本节取代旧首版“只允许管理账本出现的应用”的限制。家庭产品知识与可信变种共用；孩子明确分类独立，保存时选择孩子，可预览后复制。确定性产品列表与通用规则独立可滚动管理，无二级页签。主要类型：游戏、游戏平台/启动器、在线视频、影音播放器、其他/未知，类型不隐含配额桶。

规则支持精确产品、包、系列、验证开发者、类型和固定 all/any 组合，逐条 automatic/suggestion；名称/来源/自声明类型不能单独自动生效。优先级：孩子明确配置 > 精确产品 > 限定系列/开发者 > 通用类型；冲突保留旧有效配置，无旧配置则未归类。导入更新须预览/逐项批准，不能覆盖孩子明确配置。

Windows 已配对 Service 主动发现，运行补充便携应用；macOS 只读适配器仅显式调用。盘点不生成账本，失败不认定卸载；关联可合并/拆分/撤销，不修改历史 Segment。API/本地策略只向前生效，未知保持现有计时，本轮不改变配额键、不阻止、不部署、不升级真实安装。

> 自 ARM-D-001 起，本规格属于 `app-runtime-management/` 独立模块。Runtime 与 TimeOnChrome 主产品同仓协作但独立构建、部署和回滚；未来拆仓不改变产品/API import。
...
- D-090 覆盖上一条的“无远程日志开关”阶段限制：系统日志同时查询 accounting diagnostic 与 Windows 结构化终端日志。远程开关按机器管理，默认关闭；家长可配置最低等级、类别和不超过 7 天的 TTL，策略过期等同关闭。关闭不删除云端历史，本地有界日志仍保留，且重新打开不得补传关闭期间的日志。
