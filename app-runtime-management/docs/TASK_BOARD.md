...
- [x] 旧仓受保护交接门实际运行：`master@8b96cad` 的验真运行 `36117686246` 通过；发布运行 `36131882394` 第 3 次尝试核验原生 CI `36045220406` / SHA `575955972e64c56094f77c150548e8261c66e598`、契约锁和 MSI/Burn 哈希，并完成 `windows/x64/2.6.0/` 条件写入、逐件回读、manifest-last。首次误取 API token 作 S3 Secret、第二次 GitHub 密钥编辑未真正保存，均导致上传失败；误配 token 已吊销，用户手工保存新凭据后重跑通过。R2 latest.json 仍为 2.3.1，切换须单独审批。
- [ ] 后续：2.6.0 未签名候选仅供内部验证；终端升级和 latest 切换未获本次批准。只读跨仓令牌与 R2 凭据分别在 2026-10-25、2027-09-25 前轮换。
...
- [x] 增加 CI diff 分类器及 docs/contracts/worker/console/windows/installer/macos/release-config 路由测试。
...
- [x] Contracts 1.8.0 为 catalog item 增加兼容的 `catalogGroup/catalogGroupReasonCode`。
