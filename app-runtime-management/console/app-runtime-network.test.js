...
test('uses the explicit RuntimeSession authorization scheme', async () => {
  await requestJson({
    url: 'https://runtime.example.test/v2/module/machines',
    authorizationScheme: 'RuntimeSession',
    getToken: async () => 'opaque-session',
    fetchImpl: async (_url, options) => {
      assert.equal(options.headers.Authorization, 'RuntimeSession opaque-session');
