...
(async()=>{
 ...
 try{
  for(const viewport of [{width:1440,height:1000},{width:390,height:844}]){
   ...
   assert.match(await page.locator('#classification-rules').innerText(),/游戏建议归为受限娱乐/);
   assert.match(await page.locator('#classification-rules').innerText(),/当前孩子未批准/);
   await page.locator('#classification-rules [data-toggle-rule]').first().click();
   await page.waitForFunction(()=>document.querySelector('#classification-rules').textContent.includes('当前孩子已批准'));
   assert.match(await page.locator('#classification-rules').innerText(),/自动/);
