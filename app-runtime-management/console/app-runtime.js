(() => {
  ...
  const state = { period: 'day', offset: 0, token: null, childId: null, children: [], machines: [], users: new Map(), policy: AppRuntimePolicy.defaultPolicy(), policyEtag: '"app-policy-v0"', records: { pending: [], processed: [] }, catalog: { items: [] }, usage: {}, runtimeLogs: { range: 'today', items: [], nextCursor: null, summary: null }, timer: null, view: 'usage', appCategory: 'unclassified', actionApps: [], quotaApps: [] };
  ...
  function mockData() {
    ...
    state.catalog = { windowStartMs: Date.now() - 30 * 86400000, windowEndMs: Date.now(), items: [
      { ...state.policy.classifications[0], firstSeenAtMs: dayStart - 86400000, lastSeenAtMs: Date.now() - 300000, mainDurationMs: 4320000, machineCount: 1, userCount: 1, observedInWindow: true },
      { ...state.policy.classifications[1], firstSeenAtMs: dayStart - 86400000, lastSeenAtMs: Date.now() - 420000, mainDurationMs: 2880000, machineCount: 1, userCount: 1, observedInWindow: true },
      { ...state.policy.classifications[2], firstSeenAtMs: dayStart - 86400000, lastSeenAtMs: Date.now() - 900000, mainDurationMs: 2100000, machineCount: 1, userCount: 1, observedInWindow: true },
      { ...state.policy.classifications[3], firstSeenAtMs: null, lastSeenAtMs: null, mainDurationMs: 0, machineCount: 0, userCount: 0, observedInWindow: false },
  ...
  function categoryCounts(category) {
    const members = category === 'unclassified'
      ? state.records.pending || []
      : (state.catalog.items || []).filter((item) => item.classification === category);
  ...
  function renderAppDirectory() {
    ...
    const source = current === 'unclassified'
      ? (state.records.pending || [])
      : (state.catalog.items || []).filter((item) => item.classification === current);
