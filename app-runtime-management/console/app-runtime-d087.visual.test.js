...
(async () => {
  ...
  assert.equal(await page.locator('#game-app-count').textContent(), '2 个');
  assert.match(await page.locator('#game-app-list').textContent(), /Aimlabs.*游戏 · 未归类/s);
  assert.match(await page.locator('#game-app-list').textContent(), /Game Bar.*游戏工具 · 未归类/s);
  ...
  assert.match(await page.locator('#system-tool-list').textContent(), /系统应用 · 未归类/);
