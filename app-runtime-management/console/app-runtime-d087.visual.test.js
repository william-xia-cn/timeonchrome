...
(async () => {
  ...
  assert.equal(await page.locator('#system-app-count').textContent(), '2 个');
  assert.equal(await page.locator('#system-app-group').getAttribute('open'), null);
  await page.locator('#app-search').fill('快速助手');
  assert.notEqual(await page.locator('#system-app-group').getAttribute('open'), null);
  assert.match(await page.locator('#system-app-list').textContent(), /快速助手.*系统应用/s);
  assert.match(await page.locator('#system-app-list').textContent(), /系统应用 · 未归类/);
  assert.equal(await page.locator('#system-app-list .app-icon').first().getAttribute('aria-hidden'), 'true');
  assert.equal(await page.locator('#system-app-list .app-icon').first().evaluate((element) => getComputedStyle(element).userSelect), 'none');
  ...
  await page.locator('#system-app-list [data-classification="study"]').click();
  ...
  assert.equal(await page.locator('#system-app-count').textContent(), '1 个');
  await page.locator('#system-app-group').evaluate((element) => { element.open = true; });
  assert.match(await page.locator('#system-app-list').textContent(), /快速助手/);
  assert.deepEqual(await page.locator('[data-app-category="study"] .app-category-stat').allTextContents(),
    ['应用2', 'Windows2', 'macOS0']);
  ...
  await page.evaluate(() => {
    ...
    document.querySelector('#system-app-group').open = true;
    window.scrollTo(0, 0);
  });
  ...
  assert.equal(await qualityPage.locator('#system-app-list .product-record').count(), 1);
  assert.match(await qualityPage.locator('#system-app-list').innerText(), /记事本.*系统应用/s);
  assert.doesNotMatch(await qualityPage.locator('#system-app-list').innerText(), /1 个变体/);
  assert.equal(await qualityPage.locator('#system-app-list .product-variants').count(), 0);
