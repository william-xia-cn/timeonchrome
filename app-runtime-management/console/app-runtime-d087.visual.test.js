...
(async () => {
  ...
  assert.equal(await page.locator('#game-app-count').textContent(), '1 个');
  assert.match(await page.locator('#game-app-list').textContent(), /Aimlabs.*游戏 · 未归类/s);
  ...
  assert.match(await page.locator('#system-tool-list').textContent(), /快速助手.*系统工具/s);
  assert.match(await page.locator('#system-tool-list').textContent(), /系统工具 · 未归类/);
  ...
  await page.locator('#system-tool-group').evaluate((element) => { element.open = true; });
  ...
  assert.match(await qualityPage.locator('#system-tool-list').innerText(), /记事本.*系统工具/s);
