...
(async()=>{
  ...
  try {
    for(const viewport of [{width:1440,height:1000},{width:390,height:844}]) {
      ...
      const suite=page.locator('#managed-app-list .record-card').filter({hasText:'LibreOffice'});assert.equal(await suite.count(),1);
      await suite.locator('.product-variants summary').click();assert.match(await suite.innerText(),/LibreOffice Writer/);assert.match(await suite.innerText(),/LibreOffice Calc/);assert.match(await suite.innerText(),/LibreOffice Safe Mode/);
      assert.match(await page.locator('#inventory-status').innerText(),/registry-machine：成功.*start-menu-common：警告 1/s);
      ...
      assert.match(await page.locator('#managed-app-list').innerText(),/OBS Studio/);
      assert.equal(await page.locator('#system-app-count').innerText(),'2 个');
      await page.locator('#system-app-group').evaluate(element=>{element.open=true;});
      assert.match(await page.locator('#system-app-list').innerText(),/计算器/);
      assert.match(await page.locator('#system-app-list').innerText(),/快速助手/);
