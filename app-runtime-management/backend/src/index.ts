...
async function route(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === '/v2/auth/browser-sessions') {
    ...
    return jsonResponse(await exchangeBrowserSession(request, env, nowMs), { status: 201 });
  }
  if (url.pathname === '/v2/auth/browser-sessions/current') {
    if (request.method !== 'DELETE') return methodNotAllowed('DELETE');
    await revokeBrowserSession(request, env, nowMs);
