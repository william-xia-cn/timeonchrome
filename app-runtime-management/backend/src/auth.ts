...
export async function verifyEs256(token: string, publicJwk: string): Promise<Record<string, unknown> | null> {
  try {
...
export async function requireAccountModule(request: Request, env: Env, nowMs: number): Promise<AccountModuleClaims> {
  const runtimeSession = /^RuntimeSession ([A-Za-z0-9_-]{43,256})$/u.exec(request.headers.get('authorization') ?? '');
  if (runtimeSession?.[1]) {
    const row = await env.RUNTIME_DB.prepare(`
      SELECT account_id, children_json FROM runtime_browser_sessions_v1
      WHERE token_hash=? AND revoked_at_ms IS NULL AND expires_at_ms>?
    `).bind(await sha256Hex(runtimeSession[1]), nowMs).first<{ account_id: string; children_json: string }>();
    if (!row) throw new HttpError(401, 'UNAUTHORIZED', 'Runtime browser session is invalid or expired.');
    let children: unknown;
    try { children = JSON.parse(row.children_json); } catch { children = null; }
    const valid = Array.isArray(children) && children.length <= 100 && children.every((child) => {
    ...
    if (!valid) throw new HttpError(401, 'UNAUTHORIZED', 'Runtime browser session is invalid.');
    return {
      iss: 'runtime-browser-session', aud: 'app-runtime-management:account', sub: row.account_id,
      account_id: row.account_id, children, iat: 0, exp: Math.floor(nowMs / 1000) + 1, jti: 'runtime-session',
    } as AccountModuleClaims;
  }
  if (!env.GUARDIAN_RUNTIME_PUBLIC_JWK) {
    throw new HttpError(503, 'SERVER_MISCONFIGURED', 'Module authentication is unavailable.');
  }
