...
- App Management Console Phase B/D-085（本地完成，待单独发布授权）：`/app-runtime/` 使用与主控制台一致的外壳，顶层固定为使用统计、访问管理、应用管理、设备管理和系统管理。应用管理是无二级页签的真实应用目录与最近 30 天未归类处理；访问管理承载独立配额、默认全部开放的七天时间段和配置文件。App Policy 继续存入 `0005` 的不可变 JSON 版本表，无需新增 migration；旧策略缺失时间段时全开放，旧客户端更新不得清空已有时间段。超额和时段外只提示记录，不执行应用阻止。本阶段未部署生产环境。
