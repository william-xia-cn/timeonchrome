...
## 当前修复：应用目录折叠与 Console 响应速度

ARM-D-022 让普通应用、游戏和系统工具三个分组都可展开／折叠；折叠组不再提前创建整组产品行。Console 只渲染当前顶层页面，并并行加载互不依赖的数据，减少长目录导致的首屏和切换卡顿。该修复不改变 Worker、分类、配额、账本、Agent 或安装包。
