...
- App Management Console Phase B（本地实现完成，待单独发布授权）：`/app-runtime/` 改用与主控制台一致的左侧栏、孩子选择器和五页面信息架构；Runtime `0005` 增加孩子级版本化 App Policy、五类应用归属和独立观察型配额。分类只对设备实际应用后的新 Segment 生效，配额只显示剩余/超额，不执行应用阻止。本阶段未部署生产环境。
