...
- App Management Console Phase B/D-086（本地已补完，待单独发布授权）：`/app-runtime/` 使用与主控制台一致的外壳，顶层固定为使用统计、访问管理、应用管理、设备管理和系统管理。应用管理由当前策略与最近 30 天真实主账本合并目录驱动，使用显式分类动作；访问管理承载独立配额、默认全部开放的七天时间段和配置文件。系统管理增加基于 accounting v2 0ms diagnostic 的脱敏日志筛选、摘要和分页，不把它误报为完整 Service 文件日志。App Policy 继续存入 `0005` 的不可变 JSON 版本表，本补完无需新增 migration；超额和时段外只提示记录，不执行应用阻止。本阶段未部署生产环境。
