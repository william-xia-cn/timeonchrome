...
ARM-D-015 将可管理对象继续区分为普通应用和系统应用，并把两者与只读技术记录分开。快速助手、记事本、计算器等具有强 Windows 系统归属证据的可见应用仍可分类和配置配额；更新器、卸载器、helper、驱动入口和运行库继续只进入技术记录。系统归属不依赖名称或 Microsoft 发布者，不改变主账本、历史 Segment 或配额计算。对应 Windows 发布候选为 2.2.4，contracts 为 1.5.0。
