...
- **App Runtime 产品化状态（2026-09-01）**：D-079 的 Guardian Child-scoped 控制面、Runtime 配对/设备/统计 API、Windows WPF Setup/Agent 与独立 `/app-runtime/` 页面已实现；Product Owner 已确认满足 WiX 7 OSMF 条件并授权 `AcceptEula=wix7`。Runtime `0002`、Guardian `023`、独立 ES256 secrets、Runtime Worker `8126d3b8-27c3-4c3a-937a-cb11ac4e0ab7` 与 Guardian Worker `5dcd6678-71f9-4463-b0e4-9bff9a16eccd` 保持不变。内部未签名 Windows 1.0.1 已在 William 当前账户完成 1.0.0→1.0.1 升级：`INTELMINIPC-XW` 继续绑定 HornburgXW，DPAPI credential、设备隔离 SQLite、设备身份与 HKCU 启动项均保留，Setup 以首次 heartbeat 确认“在线 / 连接成功”。最终 MSI 为 60,144,152 bytes、SHA-256 `13b8bb04607f019acf7a9a5e68fa87f63f8075e8a3d4d4da47ddc885b635fee7`，R2 与生产下载回读一致；Pages deployment `e25c319e` 已修复北京时间范围和小时标签，真实 Chrome 显示 27 分钟及 ChatGPT/Chrome/Setup/Notepad 排行，不再显示裸 `Failed to fetch`。`BLOCKED_BY_AUTHENTICODE_SIGNING`、William 登录重启和干净账号验证仍未完成，因此不得标记为公开正式发布。Santa 与 Chrome Extension 业务保持不变。
- **App Runtime 2.0 开发目标（2026-09-01）**：Product Owner 已批准 D-080 机器级多用户管理方案。开发线将 per-user MSI/HKCU/CurrentUser DPAPI 改为管理员一次性 per-machine 安装、LocalSystem RuntimeService、ProgramData/LocalMachine DPAPI 和每交互式会话 Session Agent；云端改为 Account-scoped machine/default Child/逐用户 assignment 及 desired/applied policy version。本轮只实现与本地验证，不执行 Runtime `0003`、Guardian `024`、Worker/Pages/R2 生产发布或 William 真机升级；上述动作必须在实现证据审查后单独授权。
- **App Runtime Phase A 统一落账（2026-09-02）**：D-081 已完成实现和生产后台发布。accounting schema v2 将 `UsageSegment` 固定为唯一权威主账本，foreground/PiP 在同一用户会话与 clock epoch 内取区间并集，媒体记录独立直接求和且不进配额；Windows/macOS 共用 schema、确定性 SHA-256 和黄金向量。Runtime `0003`/`0004` 已应用，旧 v1 数据计数保持不变；Runtime Worker `af74e867-5fe1-48aa-b264-11e0892bebd3` 已部署并通过 health/未认证 fail-closed。Windows .NET 37/37、Worker 13/13、typecheck、binding type check 与 dry-run 已通过；macOS `swift test` 仍必须在 macOS 13+ 执行。未修改 Chrome Extension、Santa、配额或历史账本。
- **App Runtime Cloudflare 发布授权（2026-09-02）**：Product Owner 已明确要求完成待发布的 Cloudflare 部分。D-082 解除 D-080/D-081 的生产部署边界，授权按依赖顺序执行 Runtime `0003`/`0004`、Guardian `024`、Runtime Worker、Guardian Worker、账户级 `/app-runtime/` Pages 与经校验的 2.0.0 R2 内部包；不授权 William 真机升级、真实配对/采集、D1 历史改写、Santa/Chrome Extension 变更或未签名包公开发布。发布预检同时发现 Guardian D1 24 小时读取 28,134,760 行，其中两条逐请求审计清理 SQL 合计扫描约 28,014,688 行；该限额风险必须独立登记，不得误归因于 Runtime D1。
- **App Runtime 2.0 发布结果（2026-09-02）**：Runtime `0003`、Guardian `024`、Runtime Worker `af74e867-5fe1-48aa-b264-11e0892bebd3`、Guardian Worker `70b27ad3-7ae7-4cab-b9e0-b04def8e495b` 与 Pages `1edd8561-c990-4e6e-aa4d-b13b457cd37d` 已部署；生产回读和未认证 fail-closed 通过。当前提交重建的 2.0.0 Burn（61,110,476 bytes，SHA-256 `32a7eda83a4940948faeb034868bb0545f5984692becdb3a5187de0ba749d202`）与 MSI（60,572,816 bytes，SHA-256 `0cce622d420c6513982832005f9d02e5193a8be84171474b2cf9bb5875685e6b`）已上传生产 R2 immutable path 并回读一致。`latest.json` 仍安全保持 1.0.1：现有下载 API 硬编码直出 MSI，不能保证 Burn 的 1.x migration preflight，因此 2.0.0 latest 切换为 release blocker。William 真机升级、管理员/标准用户双账户集成、macOS 测试与 Authenticode 仍未执行。
- **App Runtime 2.0.1 收口状态（2026-09-02）**：Runtime Worker `e150e0e3-0919-4c60-be86-1ec26e4bcaf6` 已把 2.x 下载改为不可变 manifest 驱动并仅流式返回 Burn；2.0.1 Burn（60,956,054 bytes，SHA-256 `b86e8c2356fbb0730e6e1e168c48b90b38af2cd129e87f8897ec9d97f603657d`）、MSI（60,186,702 bytes，SHA-256 `e8da3c11d923d5c936581233e5827926ac70a0f5ddb12479a791c56dba31307e`）和 manifest 已上传 R2、逐项回读一致，`latest.json` 已切换至 2.0.1 Burn。WiX 7 MSI/Burn 均为 0 warning / 0 error，ProgramData DACL 仅允许 SYSTEM/Administrators，迁移在原交互式用户上下文读取 1.x CurrentUser DPAPI 后才提升安装机器 MSI。William 真机升级当前 fail closed：旧 outbox 有 9 条待上传，生产 tail 证明 Runtime 请求因账号级 D1 免费日读额度已耗尽而返回 500；代码放大根因已修复，但当日额度须等待 UTC 00:00 重置或由 Product Owner 单独批准付费升级。未 retire、未卸载、未清除旧账本。双账户、重启、真机 ACL 与 Authenticode 仍待验证。
- **App Runtime 2.0.2 安装链修正（2026-09-02）**：Workers Paid 生效后 Runtime/Guardian D1 查询恢复，William 旧 1.x outbox 已由 Agent 从 9 条清空至 0。2.0.1 首次真机安装在 Burn 调用 migration EXE 时以 `0x8000809a` fail closed；日志确认 migration 虽使用 `--self-contained true` 发布，但不是单文件，Burn 只携带 EXE 而未携带相邻 .NET runtime/DLL，因此在执行任何 retire/卸载前启动失败。旧 credential、SQLite、Agent 与云端历史均未改变。2.0.2 已改为单文件 self-contained migration，Windows 38/38 tests、隔离 package probe 与 WiX 0 warning/0 error 构建通过；Burn SHA-256 为 `ab30d11fcc296b9faf274b5c8bf46c614e509ee0629aa4ca38132202d12c9527`。William 真机重试尚在进行，2.0.1 latest 在 2.0.2 安装、生产回读通过前仍是有缺陷的内部候选。
