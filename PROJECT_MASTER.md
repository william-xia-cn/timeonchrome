...
- **App Runtime 产品化状态（2026-09-01）**：D-079 的 Guardian Child-scoped 控制面、Runtime 配对/设备/统计 API、Windows WPF Setup/Agent 与独立 `/app-runtime/` 页面已实现；Product Owner 已确认满足 WiX 7 OSMF 条件并授权 `AcceptEula=wix7`。内部未签名 MSI `1.0.0`（60,139,945 bytes，SHA-256 `847544be830979615f865667a09c690160b42381142a96cdf7174d09ff216c60`）已构建并上传独立 R2，远端回读一致；Runtime `0002`、Guardian `023`、独立 ES256 secrets、Runtime Worker `8126d3b8-27c3-4c3a-937a-cb11ac4e0ab7`、Guardian Worker `5dcd6678-71f9-4463-b0e4-9bff9a16eccd` 与 Pages deployment `81fa34db` 已部署。未创建真实 enrollment/device/segment，未安装 Agent；`BLOCKED_BY_AUTHENTICODE_SIGNING`、干净账号安装/升级/卸载、真实 module-token/首次配对仍未验证，旧 `ADMIN_API_KEY` secret 因安全审查未删除但已无路由/代码消费者。因此仍不得标记“云端和本地产品已完成”。Santa 与 Chrome Extension 业务保持不变。
