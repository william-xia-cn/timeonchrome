...
- **App Runtime Phase A 统一落账（2026-09-02）**：D-081 已在当前开发线完成本地实现和 Windows/Worker 验证。accounting schema v2 将 `UsageSegment` 固定为唯一权威主账本，foreground/PiP 在同一用户会话与 clock epoch 内取区间并集，媒体记录独立直接求和且不进配额；Windows/macOS 共用 schema、确定性 SHA-256 和黄金向量。Runtime Worker 只增加向后兼容 API、只读 accounting read model 和未应用的 additive `0004`。Windows .NET 与 Worker/D1 测试、typecheck、binding type check、dry-run 已通过；macOS `swift test` 仍必须在 macOS 13+ 执行。本轮未修改 Chrome Extension、Guardian、Santa、Pages 正式 UI、配额或生产配置，未部署且未执行远端 migration。
...
- **App Runtime 2.0 本地实现结果（2026-09-01）**：Runtime `0003` 与 Guardian `024` additive migration、Account-scoped Guardian token、Runtime v2 machine/user/policy/segment/usage/uninstall API、LocalSystem Service、无凭据 Session Agent、Pipe client PID/session/SID 校验、LocalMachine DPAPI、SID-HMAC、ProgramData machine ledger/outbox、tamper 自动恢复、WPF machine Setup、1.x migration preflight、WiX 7 per-machine MSI/Burn 和账户级 `/app-runtime/` canonical 页面已完成本地实现。MSI/Burn 均为 2.0.0 且 WiX 构建 0 warning/0 error；内部包未签名，保持 `BLOCKED_BY_AUTHENTICODE_SIGNING`。生产部署、migration、R2 上传、William 真机升级及管理员/标准用户双账户系统集成未执行。
