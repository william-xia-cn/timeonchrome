// pages-match-domain-v12-alignment.test.js
// Run with: node tests/unit/pages-match-domain-v12-alignment.test.js
...
function loadPagesMatchDomain() {
  const html = fs.readFileSync(path.join(__dirname, '..', '..', 'pages', 'index.html'), 'utf8');
  const fnSource = extractFunctionSource(html, 'matchDomain');
  const context = { URL, this: null };
  context.this = context;
  vm.runInNewContext(`${fnSource}\nthis.__fn = matchDomain;`, context, { filename: 'pages/index.html' });
  return context.__fn;
}

function run() {
  const matchDomain = loadPagesMatchDomain();
  ...
  console.log(`\n[Pages MatchDomain v1.2 Alignment] ${passed}/${total} passed${failed ? ` — ${failed} FAILED` : ''}`);
