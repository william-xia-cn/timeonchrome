// admin-match-domain-v12-alignment.test.js
// Run with: node tests/unit/admin-match-domain-v12-alignment.test.js
...
function loadAdminMatchDomain() {
  ...
  vm.runInNewContext(`${fnSource}\nthis.__fn = matchDomain;`, context, { filename: 'admin.js' });
  return context.__fn;
}

function run() {
  const matchDomain = loadAdminMatchDomain();
  ...
  console.log(`\n[Admin MatchDomain v1.2 Alignment] ${passed}/${total} passed${failed ? ` — ${failed} FAILED` : ''}`);
