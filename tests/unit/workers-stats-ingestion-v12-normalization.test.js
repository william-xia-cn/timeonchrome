...
function run() {
  ...
  expectTrue('usage-segments/v1 不应保存或返回 task 快照', !source.includes('matched_task_ids_at_time') && !source.includes('progress_task_id_at_time') && !source.includes('task_revision_at_time') && !source.includes('matchedTaskIdsAtTime'));
  expectTrue('usage-segments/v1 不应触发任务进度投影', !source.includes('rebuildTaskProgressProjectionFromSegments') && !source.includes('createTaskRepository'));
