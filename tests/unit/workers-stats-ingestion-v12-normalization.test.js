...
function run() {
  ...
  expectTrue('网站归类记录审批应支持学习/复合/退回/归为受限娱乐', siteRequestsSource.includes('normalizeSiteClassificationDecision') && siteRequestsSource.includes('decisionToStatus') && siteRequestsSource.includes("decision === 'return'") && siteRequestsSource.includes("decision === 'reject'") && siteRequestsSource.includes('customRestrictedEntertainmentList'));  expectTrue('profile 默认配置应包含 siteClassificationRulesV1', profileSource.includes('siteClassificationRulesV1: []') && profileSource.includes("'siteClassificationRulesV1'"));
