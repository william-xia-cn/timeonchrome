...
function run() {
  ...
  const migration016 = fs.readFileSync(path.join(__dirname, '..', '..', 'workers', 'migrations', '016_site_classification_record_semantics.sql'), 'utf8');
  ...
  expectTrue('016 migration 应增加记录来源、学习申请方向和聚合访问字段', migration016.includes('record_source') && migration016.includes('requested_classification') && migration016.includes('manual_requested_at') && migration016.includes('first_observed_at') && migration016.includes('last_observed_at') && migration016.includes('observation_count'));
  expectTrue('016 migration 应创建按观察源幂等累计的辅助表', migration016.includes('site_classification_observation_counters_v1') && migration016.includes('PRIMARY KEY (request_id, observation_source_id)'));
  ...
  expectTrue('网站归类记录批量上传应单条容错，避免整批 HTTP 500', siteRequestsSource.includes("code: 'SERVER_ERROR'") && siteRequestsSource.includes('site_classification_request_save_failed') && siteRequestsSource.includes('[SiteClassificationRequests] Upload item failed'));
  expectTrue('已有同目标记录应更新元数据而不是只返回旧值', siteRequestsSource.includes('mergeRequestMetadata') && siteRequestsSource.includes('record_source = CASE'));
  expectTrue('网站归类记录审批应更新 profile config/version', siteRequestsSource.includes('siteClassificationRulesV1') && siteRequestsSource.includes('version = version + 1'));
  expectTrue('网站归类记录审批应支持学习/复合/退回/归为受限娱乐/归为黑名单', siteRequestsSource.includes('normalizeSiteClassificationDecision') && siteRequestsSource.includes('decisionToStatus') && siteRequestsSource.includes("decision === 'return'") && siteRequestsSource.includes("decision === 'reject'") && siteRequestsSource.includes("decision === 'blocked'") && siteRequestsSource.includes('customRestrictedEntertainmentList') && siteRequestsSource.includes('customBlockedSites'));
  ...
  expectTrue('usage-segments/v1 应保存并返回 task 快照', source.includes('matched_task_ids_at_time = ?') && source.includes('progress_task_id_at_time = ?') && source.includes('task_revision_at_time = ?') && source.includes('matchedTaskIdsAtTime: parseJsonField(row.matched_task_ids_at_time)'));
  expectTrue('usage-segments/v1 应在新 segment 接受后触发任务进度投影', source.includes("import { createTaskRepository } from '../tasks/taskRepository'") && source.includes('rebuildTaskProgressProjectionFromSegments') && source.includes('if (progressTaskIdAtTime && taskRevisionAtTime)') && source.indexOf('rebuildTaskProgressProjectionFromSegments') > source.indexOf('inserted++;'));
