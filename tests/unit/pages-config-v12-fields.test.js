...
function run() {
  ...
  const navOrder = ['data-page="stats"', 'data-page="rules"', 'data-page="review"', 'data-page="devices"', 'data-page="account"', 'data-page="system-management"'].map(item => source.indexOf(item));
  expectTrue('Pages 左侧导航顺序应为使用统计/访问管理/网站归类申请/设备管理/用户管理/系统管理', navOrder.every(i => i >= 0) && navOrder.every((value, index) => index === 0 || value > navOrder[index - 1]));
  expectTrue('Pages 账户设置可见文案应改为用户管理', source.includes('用户管理') && !source.includes('<span class="nav-icon">👤</span><span>账户设置</span>'));
  ...
  expectTrue('Pages 系统管理应包含统计对账/网页落账/媒体落账/系统日志/数据备份与恢复 Tab', source.includes('data-system-management-tab="reconciliation"') && source.includes('data-system-management-tab="web-settlements"') && source.includes('data-system-management-tab="media-settlements"') && source.includes('data-system-management-tab="client-logs"') && source.includes('data-system-management-tab="backup-restore"'));
  ...
  expectTrue('Pages 设备管理应保持顶层页面而不是系统管理子页面', devicesPageStart > systemPageStart && !systemPageSlice.includes('id="page-devices"'));
  expectTrue('Pages 系统管理仍应包含全部内部面板', systemPageSlice.includes('data-system-management-panel="reconciliation"') && systemPageSlice.includes('data-system-management-panel="web-settlements"') && systemPageSlice.includes('data-system-management-panel="media-settlements"') && systemPageSlice.includes('data-system-management-panel="client-logs"') && systemPageSlice.includes('data-system-management-panel="backup-restore"'));
