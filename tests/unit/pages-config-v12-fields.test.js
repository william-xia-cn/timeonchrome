...
function run() {
  ...
  expectTrue('Pages 每日配额应显式包含在线总额', extractFunctionSource(source, 'renderQuotaPage').includes('onlineMinutes') && extractFunctionSource(source, 'saveTimeQuotaConfig').includes('onlineMinutes'));
  expectTrue('Pages 七天计划合计应明确无限制日且不再伪装成周上限', source.includes('七天每日计划合计') && extractFunctionSource(source, 'updateWeeklyQuotaTotals').includes('包含无限制日'));
  expectTrue('Pages 配额导入导出应包含周休息上限和每日在线总额', extractFunctionSource(source, 'buildProfileConfigExportData').includes('weekly: { restMinutes: weeklyRestQuotaView(cfg).value }') && source.includes("{ key: 'onlineMinutes', label: '在线总额' }") && extractFunctionSource(source, 'buildProfileConfigImportPayload').includes("diff.area === 'weekly-quota'"));
  ...
  expectTrue('Pages 自主度配置应参与导入导出和差异应用', extractFunctionSource(source, 'buildProfileConfigExportData').includes('autonomyConfig:') && extractFunctionSource(source, 'normalizeProfileConfigImportData').includes('importedAutonomy') && extractFunctionSource(source, 'buildProfileConfigImportPayload').includes("diff.area === 'autonomy'"));
  expectTrue('Pages 消息通知应拆分账号通道与档案阈值', source.includes('id="account-notification-email-enabled"') && source.includes('id="account-notification-telegram-enabled"') && source.includes('id="unclassified-notification-threshold"'));
  expectTrue('Pages 消息通知应使用账号通道和档案功能独立 API', source.includes('/account/notification-settings/v1') && source.includes('/unclassified-usage-notification/v1'));
  expectTrue('Pages profile 配置导出只包含档案通知功能，不包含账号通道', extractFunctionSource(source, 'buildProfileConfigExportData').includes('unclassifiedUsage') && !extractFunctionSource(source, 'buildProfileConfigExportData').includes('emailEnabled') && !extractFunctionSource(source, 'buildProfileConfigExportData').includes('telegramEnabled'));
  ...
  expectTrue('Pages 应包含网站归类记录统一入口', source.includes('<h2>网站归类记录</h2>') && source.includes('site-classification-requests/v1') && source.includes('/used-unclassified-sites/v1?days=30'));
  expectTrue('Pages 网站归类记录页面应只保留两个主单元', source.includes('复合网站申请学习记录') && source.includes('未归类网站使用记录') && source.includes('id="site-classification-review-card"') && source.includes('id="review-used-unclassified-card"') && source.includes('data-review-used-unclassified="true"'));
  expectTrue('Pages 网站归类记录不应保留审核命名和旧日期会话块', !source.includes('网站归类审核') && !source.includes('网站归类申请审核') && !source.includes('待审核会话') && !source.includes('review-date') && !source.includes('review-groups-container') && !source.includes('/pending-reviews?date='));
  const loadSiteClassificationRequestsSource = extractFunctionSource(source, 'loadSiteClassificationRequests');
  const renderReviewUsedUnclassifiedSitesSource = extractFunctionSource(source, 'renderReviewUsedUnclassifiedSites');
  const renderUnclassifiedUsageRecordsSource = extractFunctionSource(source, 'renderUnclassifiedUsageRecords');
  ...
  expectTrue('Pages 复合网站申请学习记录应读取全部显式记录再仅展示学习申请', loadSiteClassificationRequestsSource.includes('/site-classification-requests/v1?status=all') && extractFunctionSource(source, 'renderLearningRequestRecords').includes("siteRequestRecordKind(request) === 'learning_request'"));
  expectTrue('Pages 未归类网站使用记录应合并自动记录和最近 30 天聚合', renderReviewUsedUnclassifiedSitesSource.includes('explicitUnclassifiedRequestEntries(requests)') && renderReviewUsedUnclassifiedSitesSource.includes('reviewUsedUnclassifiedEntries()') && renderUnclassifiedUsageRecordsSource.includes('buildUnclassifiedReviewRows(requests, usedEntries)') && source.includes("type: 'request'") && source.includes("type: 'used'"));
  expectTrue('Pages 未归类网站使用记录应使用统一列表行而非审批四列布局', source.includes('function renderUnclassifiedReviewRow') && renderUnclassifiedUsageRecordsSource.includes('renderUnclassifiedReviewRow(row, rows.indexOf(row))') && !renderUnclassifiedUsageRecordsSource.includes('renderSiteRequestRow(row.request, row.idx)'));
  expectTrue('Pages 两个记录单元应默认显示未处理并折叠已处理记录', source.includes('record-history-details') && source.includes('已处理记录') && source.includes('isProcessedSiteRequest') && source.includes('pendingRows') && source.includes('processedRows'));
  expectTrue('Pages 已使用未归类网站归类前应先生成或复用审核记录', source.includes('/site-classification-requests/v1/ensure') && source.includes('decideSiteClassificationRequestRecord') && classifyReviewUsedUnclassifiedSource.includes('classifyUnclassifiedEntryViaRequest') && !classifyReviewUsedUnclassifiedSource.includes("/used-unclassified-sites/v1', 'POST'"));
  ...
  expectTrue('Pages 审核 badge 应按 canonical host 合并网站归类记录和已使用未归类网站', extractFunctionSource(source, 'updateReviewBadge').includes('siteResult') && extractFunctionSource(source, 'updateReviewBadge').includes('usedUnclassifiedResult') && extractFunctionSource(source, 'updateReviewBadge').includes('pendingRequestKeys') && extractFunctionSource(source, 'updateReviewBadge').includes('usedOnlyKeys') && !extractFunctionSource(source, 'updateReviewBadge').includes('pending-reviews'));
  ...
  expectTrue('Pages 应为访问记录提供确认/暂不归类/黑名单动作', source.includes('确认为学习网站') && source.includes('确认为复合网站') && source.includes('暂不归类') && source.includes('归为受限娱乐') && source.includes('归为黑名单'));
  ...
  expectTrue('Pages 网站归类记录应兼容缺 recordSource 的旧自动访问记录', source.includes('function hasSiteRequestObservationEvidence') && source.includes("request.recordSource === 'legacy') && hasSiteRequestObservationEvidence"));
  expectTrue('Pages 移动端归类记录应使用单列记录和两列动作', source.includes('grid-template-columns: minmax(0, 1fr)') && source.includes('.site-request-actions .btn-classify') && source.includes('grid-template-columns: 1fr 1fr'));
