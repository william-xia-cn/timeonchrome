...
function run() {
  ...
  expectTrue('Pages 未归类网站使用记录应合并自动记录和最近 30 天聚合', renderReviewUsedUnclassifiedSitesSource.includes('explicitUnclassifiedRequestEntries(requests)') && renderReviewUsedUnclassifiedSitesSource.includes('reviewUsedUnclassifiedEntries()') && renderUnclassifiedUsageRecordsSource.includes('type: "request"') && renderUnclassifiedUsageRecordsSource.includes('type: "used"'));
  expectTrue('Pages 未归类网站使用记录应使用统一列表行而非审批四列布局', source.includes('function renderUnclassifiedRequestRow') && renderUnclassifiedUsageRecordsSource.includes('renderUnclassifiedRequestRow(row.request, row.idx)') && !renderUnclassifiedUsageRecordsSource.includes('renderSiteRequestRow(row.request, row.idx)'));
  ...
  expectTrue('Pages 审核 badge 应统计网站归类记录和已使用未归类网站', extractFunctionSource(source, 'updateReviewBadge').includes('siteResult') && extractFunctionSource(source, 'updateReviewBadge').includes('usedUnclassifiedResult') && !extractFunctionSource(source, 'updateReviewBadge').includes('pending-reviews'));
