...
function run() {
  ...
  expectTrue('popup local snapshot defines special site targets before returning snapshot', backgroundSource.includes('const specialSiteTargets = getTabSpecialSiteTargets') && backgroundSource.includes('specialSiteTargets,'));
  expectTrue('background defines special site context remember helper', backgroundSource.includes('function rememberTabSpecialSiteContext'));
  expectTrue('background defines special site context read helper', backgroundSource.includes('function getTabSpecialSiteTargets'));
  expectTrue('background defines special site context clear helper', backgroundSource.includes('function clearTabSpecialSiteContext'));
  expectTrue('popup settled mode stats prefer target classification display before quota fallback', backgroundSource.includes('dayStats?.targets') && backgroundSource.includes('popupClassificationCategory') && backgroundSource.includes('activeByQuotaBucket'));
  ...
  expectTrue('popup keeps dry-run validation before final learning request submit', popupJs.includes('async function validateSiteClassificationRequestInput') && /validateSiteClassificationRequestInput\(value, defaults\.sourceTabId, defaults\.specialSiteTargets \|\| \[\]\)[\s\S]*SUBMIT_SITE_CLASSIFICATION_REQUEST/.test(popupJs));
  ...
  expectTrue('popup YouTube special request panel uses compact card UI', popupHtml.includes('id="site-request-special-summary"') && popupHtml.includes('.request-special-options button.active') && popupHtml.includes('.request-panel[data-special-site-request="youtube"]') && popupJs.includes('申请 YouTube 学习对象') && popupJs.includes('已识别：'));
  expectTrue('popup YouTube channel option comes from page context only', popupJs.includes("setSiteRequestPanelMode(defaults.input || '', defaults.specialSiteTargets || [])") && popupJs.includes('function youtubeChannelOptionFromSpecialTargets') && popupJs.includes('const contextChannel = youtubeChannelOptionFromSpecialTargets(specialSiteTargets)') && !popupHtml.includes('.request-special-options button:disabled'));
  expectTrue('popup YouTube special request defaults to single video when present', popupJs.includes('function getDefaultYouTubeSpecialRequestOptionIndex') && popupJs.includes("item?.kind === 'video'") && popupJs.includes('idx === defaultIndex') && popupJs.includes('renderSpecialSiteRequestSummary(defaultIndex)'));
  expectTrue('popup site request previews YouTube playlist/video canonical target', popupJs.includes('YouTube 播放列表') && popupJs.includes('https://www.youtube.com/playlist?list=') && popupJs.includes('确认对象：YouTube 播放列表') && popupJs.includes('确认对象：YouTube 单个视频') && popupJs.includes('https://www.youtube.com/watch?v='));
  ...
  expectTrue('admin rules display keeps user custom site group', adminJs.includes('用户自定义配置') && adminJs.includes('来自当前孩子档案的自定义网站配置，本机不可修改') && adminJs.includes('config?.customStudyList') && adminJs.includes('config?.customCompositeList') && adminJs.includes('config?.customRestrictedEntertainmentList') && adminJs.includes('config?.customBlockedSites'));
  ...
  expectTrue('GET_CLOUD_STATUS also reads cloud-sync runtime state', messageRouterSource.includes('const syncStateRef = getSyncState()') && messageRouterSource.includes("!!storage['cloud_device_token'] || !!syncStateRef.deviceToken") && messageRouterSource.includes("'cloud_connection_state_v1'"));
