...
function run() {
  ...
  expectTrue('admin local mode retains read-only Access Management navigation', !extractFunctionSource(code, 'enterLocalReadOnlyMode').includes("page === 'rules'") && !extractFunctionSource(code, 'refreshPageByNav').includes("isLocalReadOnlyMode && page === 'rules'"));
  ...
  expectTrue('admin 访问管理页应拆分为五个页内 Tab', html.includes('data-rules-tab="site-management"') && html.includes('data-rules-tab="quota-management"') && html.includes('data-rules-tab="autonomy-management"') && html.includes('data-rules-tab="schedule-management"') && html.includes('data-rules-tab="classification-requests"') && html.includes('网站管理') && html.includes('时间配额') && html.includes('自主度配置') && html.includes('时间段管理') && html.includes('网站归类记录'));
  expectTrue('admin 访问管理页应包含五个内容面板', html.includes('data-rules-panel="site-management"') && html.includes('data-rules-panel="quota-management"') && html.includes('data-rules-panel="autonomy-management"') && html.includes('data-rules-panel="schedule-management"') && html.includes('data-rules-panel="classification-requests"'));
  ...
  expectTrue('admin 本地自主度页应只读显示进入确认和休息软限额', html.includes('data-rules-panel="autonomy-management"') && html.includes('id="rules-autonomy-entry-display"') && html.includes('id="rules-rest-reminder-display"') && code.includes('function getAdminAutonomyView') && code.includes('restrictedEntryConfirmationRequired') && code.includes('softReminderTimeoutAction') && code.includes('今日休息软限额') && code.includes('超额后提醒间隔'));
  expectTrue('admin 本地自主度页不应渲染禁用输入或保存按钮', !html.includes('id="save-autonomy-btn"') && !html.includes('id="a-restricted-entry-confirmation"') && !html.includes('id="a-rest-reminder-enabled"'));
  expectTrue('admin 本地提醒说明应明确账本和两种超时口径', code.includes('软限额只做提醒，不会锁定网站访问') && code.includes('Rest 配额网页账本') && code.includes('媒体时长不计入') && code.includes('3 分钟结算周期') && code.includes("默认继续休息' : '结束休息'"));
  expectTrue('admin 本地时间段同时展示允许窗口和锁定补集', code.includes('function formatLockedWindowsLabel') && code.includes('<span>允许：${formatWindowsLabel(windows)}</span>') && code.includes('锁定：${formatLockedWindowsLabel(windows)}') && code.includes('computeOnlineWindowsForDay'));
  expectTrue('admin 本地访问管理手机端使用纵向网格而非强制横向缩放', html.includes('@media (max-width: 720px)') && html.includes('.rules-grid-row.header { display: none; }') && html.includes('grid-template-columns: repeat(2, minmax(0, 1fr))') && !html.includes('.rules-schedule-grid { overflow-x: auto; }'));
  ...
  expectTrue('admin 应明确显示已由当前配置终结的本地归类记录', code.includes("record?.syncStatus === 'resolved'") && code.includes('当前配置已归类'));
