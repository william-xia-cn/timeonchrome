...
function run() {
  ...
  // 背景链路：monitoring_enabled=0 时短路 heartbeat/quota_check/checkAutoStudy
  expectTrue('background: heartbeat 分支应有 monitoring guard', /if \(alarm\.name === 'heartbeat'\) \{\s*if \(!isMonitoringEnabled\(\)\) return;\s*await heartbeat\(\);/s.test(bg));
