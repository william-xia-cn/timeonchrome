...
export function initSignal(onContextChange) {
  ...
  function mergeEvent(pending, incoming) {
    return {
      ...
      isWindowFocused: incoming.isWindowFocused ?? pending.isWindowFocused,
      windowState: incoming.windowState ?? pending.windowState,
      evidenceTier: incoming.evidenceTier ?? pending.evidenceTier,
      documentVisible: incoming.documentVisible ?? pending.documentVisible,
  ...
  chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    ...
    if (hasAudibleFact) {
      ...
      onEvent({
        ...
        isWindowFocused: focus.isFocused === true,
