...
async function buildSettledSegmentsPayload(segments) {
...
async function performSend(options) {
  ...
  try {
    const payload = options.type === 'settledUsageSegments'
      ? await buildSettledSegmentsPayload(options.segments || [])
      : await buildHeartbeatPayload(options);
...
function drainQueuedSend() {
  ...
  if (queuedSegmentBatches.length > 0) {
    const queued = queuedSegmentBatches.shift();
    startSend(queued.options).then(queued.resolve, queued.resolve);
    return;
  }
  if (queuedHeartbeat) {
...
export async function mirrorPersistedUsageSegments(segments) {
  ...
  return new Promise((resolve) => {
    queuedSegmentBatches.push({ options, resolve });
    if (queuedSegmentBatches.length > 20) {
      const dropped = queuedSegmentBatches.shift();
      dropped?.resolve({ ok: false, skipped: true, errorCode: 'native_queue_full' });
    }
  });
}
