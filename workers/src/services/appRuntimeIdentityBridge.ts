import {
  ...
  APP_RUNTIME_SSO_AUDIENCE,
  type AppRuntimeChildLifecycleClaims,
  type AppRuntimeModuleClaims,
  type AppRuntimeAccountModuleClaims,
  type AppRuntimeSsoLaunchResponse,
  type AppRuntimeSsoTicketClaims,
} from '@timeonchrome/app-runtime-contracts';
...
export async function handleAppRuntimeAccountToken(request: Request, env: RuntimeBridgeEnv): Promise<Response> {
  ...
  try {
    return json({ token: await signJwt(env.APP_RUNTIME_TOKEN_PRIVATE_JWK, claims as unknown as Record<string, unknown>), expiresAt: claims.exp, audience: claims.aud });
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : 'token_issue_failed' }, 503);
  }
}

async function signJwt(privateJwk: string | undefined, claims: Record<string, unknown>): Promise<string> {
  if (!privateJwk) throw new Error('app_runtime_identity_bridge_not_configured');
  const key = await crypto.subtle.importKey(
    'jwk', JSON.parse(privateJwk),
...
export async function handleAppRuntimeSsoTicket(request: Request, env: RuntimeBridgeEnv): Promise<Response> {
  if (request.method !== 'POST') return json({ error: 'Method not allowed' }, 405);
  const accountId = await verifyAccountToken(request, env.JWT_SECRET);
  if (!accountId) return json({ error: 'Unauthorized' }, 401);
  const result = await env.DB.prepare(`
  ...
  try {
    const ticket = await signJwt(env.APP_RUNTIME_SSO_PRIVATE_JWK, claims as unknown as Record<string, unknown>);
    const runtimeOrigin = new URL(env.APP_RUNTIME_CONSOLE_ORIGIN || 'https://timeonchrome-app-runtime-console.pages.dev').origin;
    const response: AppRuntimeSsoLaunchResponse = {
      launchUrl: `${runtimeOrigin}/#ticket=${encodeURIComponent(ticket)}`,
      expiresAt: claims.exp,
      audience: APP_RUNTIME_SSO_AUDIENCE,
    };
    return json(response);
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : 'sso_ticket_issue_failed' }, 503);
  }
}
...
export async function processAppRuntimeLifecycleOutbox(env: RuntimeBridgeEnv): Promise<void> {
  ...
  for (const row of pending.results || []) {
    ...
    try {
      const token = await signJwt(env.APP_RUNTIME_TOKEN_PRIVATE_JWK, claims as unknown as Record<string, unknown>);
