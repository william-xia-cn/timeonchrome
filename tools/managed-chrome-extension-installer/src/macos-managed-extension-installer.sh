...
DEPLOYMENT_NAME="ManagedChromeExtension"

say() { /bin/echo "[$DEPLOYMENT_NAME] $*"; }
fail() { /bin/echo "[$DEPLOYMENT_NAME] ERROR: $*" >&2; exit 1; }
...
config_optional_bool() {
  local value default_value
  default_value="${2:-false}"
  value="$(/usr/bin/plutil -extract "$1" raw -n "$CONFIG_FILE" 2>/dev/null || true)"
  case "$value" in
    true|1|yes) /bin/echo true ;;
    false|0|no) /bin/echo false ;;
    "") /bin/echo "$default_value" ;;
    *) fail "Config key $1 must be boolean." ;;
  esac
}
...
DEPLOYMENT_NAME="$(config_optional_string deploymentName ManagedChromeExtension)"
POLICY_SLUG="$(config_optional_string policySlug managed-chrome-extension)"
DAEMON_LABEL="$(config_optional_string launchDaemonLabel "local.$POLICY_SLUG.restore-chrome-policy")"
APP_SUPPORT_NAME="$(config_optional_string appSupportName "$DEPLOYMENT_NAME")"
TARGET_EMAIL="$(config_string targetProfileEmail)"
TARGET_LABEL="$(config_optional_string managedDeviceLabel "")"
if [ -z "$TARGET_LABEL" ]; then TARGET_LABEL="$(config_string deviceLabel)"; fi
...
MCX_SRC="$POLICY_ROOT/managed-extension-mcx.plist"
MANAGED_PREF_DOMAIN="com.google.Chrome.extensions.$EXTENSION_ID"
MANAGED_PREF_SRC="$POLICY_ROOT/managed-extension-preferences.plist"
RESTORE_SCRIPT="/usr/local/sbin/$POLICY_SLUG-restore-chrome-policy.sh"
LAUNCH_DAEMON="/Library/LaunchDaemons/$DAEMON_LABEL.plist"
...
BACKUP_ROOT="/Library/Application Support/$APP_SUPPORT_NAME/private-deployment/$EXTENSION_ID"
...
print(matches[0])
PY
)" || fail "Exactly one Chrome Profile must match the configured profile email."
...
validate_update_feed() {
  [ "$VALIDATE_UPDATE_FEED" = true ] || { say "Update feed preflight is disabled by config."; return 0; }
  local feed status
  feed="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-feed.XXXXXX")"
  register_temp "$feed"
  /bin/chmod 600 "$feed"
...
validate_api_token() {
  [ "$VALIDATE_CLOUD_TOKEN" = true ] || { say "Cloud token preflight is disabled by config."; return 0; }
  local curl_config response status
  curl_config="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-curl.XXXXXX")"
  response="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-response.XXXXXX")"
...
PY
  cloud_config_path="$(config_optional_string cloudConfigPath /device/config)"
  status="$(/usr/bin/curl --silent --show-error --connect-timeout 15 --max-time 30 --config "$curl_config" --output "$response" --write-out '%{http_code}' "$CLOUD_ENDPOINT$cloud_config_path")" || fail "Cloud token preflight request failed."
...
payload = {domain: {
    ...
    "managedDeviceLabel": always(cfg.get("managedDeviceLabel") or cfg["deviceLabel"]),
...
payload = {
    ...
    "managedDeviceLabel": cfg.get("managedDeviceLabel") or cfg["deviceLabel"],
...
install_keeper_assets() {
  ...
  policy_tmp="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-policy.XXXXXX")"
  mcx_tmp="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-mcx-source.XXXXXX")"
  managed_pref_tmp="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-managed-pref.XXXXXX")"
  ...
  /usr/bin/install -d -m 755 "/Library/Managed Preferences" "$POLICY_ROOT" "/usr/local/sbin"
...
#!/bin/bash
set -euo pipefail
SRC="__POLICY_ROOT__/com.google.Chrome.plist"
MCX_SRC="__POLICY_ROOT__/managed-extension-mcx.plist"
MANAGED_SRC="__POLICY_ROOT__/managed-extension-preferences.plist"
...
LOG="/var/log/managed-chrome-extension-policy-restore.log"
COMPUTER_RECORD="/Computers/local_computer"
timestamp() { /bin/date "+%Y-%m-%d %H:%M:%S"; }
log() { /bin/echo "$(timestamp) $1" >> "$LOG"; /usr/bin/logger -t managed-chrome-extension-policy "$1"; }
...
mcx_dump="$(/usr/bin/mktemp /private/tmp/managed-extension-mcx-read.XXXXXX)"
...
KEEPER
  /usr/bin/python3 - "$RESTORE_SCRIPT" "$POLICY_ROOT" <<'PY'
import sys
path, policy_root = sys.argv[1:]
with open(path, encoding="utf-8") as fh:
    text = fh.read()
text = text.replace("__POLICY_ROOT__", policy_root)
...
<plist version="1.0"><dict>
  ...
  <key>StandardOutPath</key><string>/var/log/managed-chrome-extension-policy-restore.out.log</string>
  <key>StandardErrorPath</key><string>/var/log/managed-chrome-extension-policy-restore.err.log</string>
</dict></plist>
EOF
...
install_managed_activation() {
  ...
  /usr/bin/dscl /Local/Default -create "$COMPUTER_RECORD" HardwareUUID "$hardware_uuid"
  mcx_tmp="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-mcx.XXXXXX")"
  register_temp "$mcx_tmp"
  build_mcx_plist "$mcx_tmp"
...
verify_clean_state() {
  ...
  for path in "$POLICY_SRC" "$MCX_SRC" "$MANAGED_PREF_SRC" "$RESTORE_SCRIPT" "$LAUNCH_DAEMON" "$POLICY_DST" "$MANAGED_PREF_DST"; do
    [ ! -e "$path" ] || fail "Clean-state verification found a remaining managed deployment asset."
  done
  ...
  if mcx_domain_present; then fail "Clean-state verification found the managed extension MCX domain."; fi
...
validate_hardening_policy() {
  /usr/bin/python3 - "$POLICY_SRC" "$EXTENSION_ID" "$TARGET_EMAIL" "$ENABLE_HARDENING" <<'PY'
import plistlib, re, sys
with open(sys.argv[1], "rb") as fh:
    p = plistlib.load(fh)
eid, email, hardening = sys.argv[2:]
...
validate_mcx() {
  ...
  dump="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-mcx-read.XXXXXX")"
  expected_source="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-mcx-expected.XXXXXX")"
  register_temp "$dump"
  ...
  /usr/bin/dscl /Local/Default -mcxread "$COMPUTER_RECORD" > "$dump" 2>/dev/null || fail "Unable to read the managed extension policy."
...
PY
  ...
  /usr/bin/mcxquery -user "$TARGET_USER" -format space | /usr/bin/grep -Fq "com.google.Chrome.extensions.$EXTENSION_ID" || fail "The managed extension policy is not effective for the target user."
  say "Managed storage fields, Token shape, Hardware UUID, and effective MCX are valid."
}

validate_managed_preferences_files() {
  local expected
  expected="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-managed-pref-expected.XXXXXX")"
...
validate_installed_extension() {
  local manifest
  manifest="$(installed_manifest_path)" || fail "The expected extension version is not installed in the target Profile."
  [ "$VALIDATE_MANAGED_SCHEMA" = true ] || { say "Managed schema validation is disabled by config."; return 0; }
  /usr/bin/python3 - "$CONFIG_FILE" "$manifest" <<'PY'
...
if not schema:
    raise SystemExit(1)
path = os.path.join(os.path.dirname(sys.argv[2]), schema)
with open(path, encoding="utf-8") as fh:
    s = json.load(fh)
props = s.get("properties", {})
required = cfg.get("requiredManagedKeys") or ["enabled", "deploymentMode", "managedDeviceToken", "managedDeviceLabel", "managedProfileEmail", "allowIdentityRecovery"]
if cfg.get("cloudEndpoint") and "cloudEndpoint" not in required:
    required.append("cloudEndpoint")
if s.get("type") != "object" or any(key not in props for key in required):
    raise SystemExit(1)
PY
...
validate_chrome_managed_storage() {
  ...
  [ -d "$storage_root" ] || fail "Chrome has not created managed storage for the extension."
  /usr/bin/python3 - "$CONFIG_FILE" "$storage_root" <<'PY'
...
if cfg.get("cloudEndpoint") and "cloudEndpoint" not in required:
    required.append("cloudEndpoint")
needles = list(required) + [cfg["managedDeviceToken"], cfg["targetProfileEmail"]]
if cfg.get("cloudEndpoint"):
    needles.append(cfg["cloudEndpoint"])
blob = b""
for name in os.listdir(root):
    ...
    if os.path.isfile(path):
        with open(path, "rb") as fh:
            blob += fh.read()
for item in needles:
    if str(item).encode() not in blob:
        raise SystemExit(1)
PY
...
validate_installation() {
  ...
  validate_system_installation
  validate_installed_extension
  say "Complete installation validation passed."
}
...
run_restore_test() {
  ...
  safety="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-restore-safety.XXXXXX")"
  managed_safety="$(/usr/bin/mktemp "$TEMP_ROOT/managed-extension-managed-restore-safety.XXXXXX")"
...
ensure_first_install_target_is_clean() {
  local path
  for path in "$POLICY_SRC" "$MCX_SRC" "$MANAGED_PREF_SRC" "$RESTORE_SCRIPT" "$LAUNCH_DAEMON" "$POLICY_DST" "$MANAGED_PREF_DST"; do
    [ ! -e "$path" ] || fail "Existing local Chrome policy assets were found; use install repair only for an already validated installation."
  done
  if mcx_domain_present; then fail "An existing managed extension MCX domain was found; refusing to overwrite it during first install."; fi
}
...
install_all() {
  ...
  if installation_state_present; then
    say "Existing or partial policy state detected; entering repair/reinstall flow."
    reinstall_all
    return $?
  fi
  ensure_first_install_target_is_clean
  ...
  if wait_for_complete_validation; then
    say "Installation and validation completed successfully."
  else
    rc=$?
    [ "$rc" -eq 2 ] || fail "Validation failed unexpectedly; installed policy was retained."
    say "System policy was installed, but managed activation was not accepted as complete."
  fi
}
...
reinstall_all() {
  ...
  open_chrome
  if wait_for_complete_validation; then
    say "Repair/reinstall and validation completed successfully."
  else
    rc=$?
    [ "$rc" -eq 2 ] || fail "Validation failed unexpectedly; installed policy was retained."
    say "Repair/reinstall policy was installed, but managed activation was not accepted as complete."
  fi
}
