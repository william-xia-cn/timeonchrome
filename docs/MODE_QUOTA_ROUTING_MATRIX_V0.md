...
- Rest usage checkpoint is a post-access UI responsibility, not a fourth mode or an access-route Reminder. It reads only the settled Rest quota-bucket web ledger. `restConfig.firstReminderMinutes` is the configurable daily Rest soft limit (default 120 minutes, `null` disables), and `restConfig.repeatReminderMinutes` is the configurable overage reminder interval (default 60 minutes). The in-page prompt keeps the web ledger running, excludes media stats, and starts its 60-second response deadline only after Content confirms the prompt is visible. `autonomyConfig.softReminderTimeoutAction` defaults to `end_rest`; `continue` dismisses the visible prompt, resumes paused media and advances the next threshold. Failed delivery retries once; the second failure enters the full Reminder only for `end_rest`, while `continue` records the missed reminder and advances without blocking.
- `autonomyConfig.restrictedEntryConfirmationRequired` defaults to `true`. It is consulted only after the existing route has already established that the target is restricted and Rest schedule/quota/domain limits permit entry. `true` preserves `to_rest_slide_confirm` / restricted `to_rest_confirm`; `false` commits the same Rest transition immediately and emits a short non-blocking notice. It never changes blocked, schedule-locked, quota-exhausted, composite, pending or Rest-grace branches.
