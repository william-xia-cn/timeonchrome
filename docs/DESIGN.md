...
## 6.3 任务管理 V1（Approved，隔离分支实现与测试已收口）
...
当前状态为 Approved，正在隔离分支 `codex/task-management-v1` 按 D-058 重构。目标是可整体拔除的独立 Task 模块：扩展宿主只提供不含 Task 语义的 optional-module host；Task 自己拥有资源解析、cache、同步、访问前置判断、进度账本、消息协议与独立页面。受 Chrome MV3 module Service Worker 禁止运行期 `import()` 的限制，Task 由 `background.js` 中唯一一行静态 side-effect import 安装；测试移除 Task 时必须同时移除该安装行和 Task 目录。关闭安装行后，mode-service、核心计时/落账、cloud-sync、Popup、Admin、Reminder 和 Pages 主界面均保持原状。任务资源命中只允许继续执行原访问管理流程，不绕过黑名单、配额或时间段。Worker 继续部署在 guardian-api，但 Task router/repository/schema/capability 独立，不再修改 devices 表或从核心 usage segment 投影进度。任务管理 V1 仍不属于 `master` 当前运行基线；生产 D1 migration、Worker/Pages 部署、扩展版本升级和发布均未执行。

Task 资源协议按 D-059 固定为 `hosts + urlRules + specialTargets`。域名规则覆盖自身及全部子域；URL 规则必须显式使用 `exact` 或 `path_prefix`，前者忽略 hash、尾斜杠和 tracking query 但保留业务 query，后者忽略 query 并按同 host 的路径段边界匹配。旧 `urls` 只在 normalization 层按 `exact` 兼容读取，canonical cache 与 Worker 存储只输出 `urlRules`。YouTube URL 在 Task 模块内部识别为视频、播放列表或频道，不调用访问管理特殊对象解析器。扩展与 Worker 各自实现同一协议，并由 `tests/fixtures/task-resource-canonical-v1.json` 验证输出一致。
