...
当前状态为 Approved，尚未实现：仓库尚未实现任务表、任务 API、设备任务同步、任务运行时策略、任务进度投影或相关 UI。技术设计采用独立 `tasks_v1` / `task_events_v1`，并让现有 `usage_segments_v1` 承担任务有效使用时间的唯一事实；这些内容进入隔离分支 `codex/task-management-v1` 开发，不属于 `master` 当前运行基线。
