...
| 综合 | 学习网站 | 学习 | 自动回归 | 极轻 / 无 | studySeconds |
| 综合 | 综合网站 | 综合 | 继续使用 | 无 / 轻 | compositeSeconds |
| 综合 | 休息网站 | 休息 | 普通确认 | 中 | restSeconds |
| 综合 | hardBlocked | 无 | 拦截 | 高 | 不计入有效时间 |
| 休息 | 学习网站 | 学习 | 自动回归 | 极轻 / 无 | studySeconds |
| 休息 | 综合网站 | 综合 | 自动切换 + 轻提示 | 低 | compositeSeconds |
...
当前模式：{学习 / 综合 / 休息 / 暂停}
当前网站：{domain}
当前计时：{currentSessionDuration}

今日剩余：
学习：不限
综合：{remainingCompositeTime}
休息：{remainingRestTime}
```
