...
- **高风险分类硬约束**：Worker、终端运行时归一化和本地 fallback 共同保护云游戏、门户游戏/视频/直播子站及短视频站点，错误移动由服务端拒绝。
- **单一权威与并发保护**：Profile 不再持久化系统默认/effective 清单；系统配置写入必须携带 expected version，并保存不可变版本历史和 SHA-256 摘要。
- **可追溯历史更正**：新增逐 segment correction ledger 及独立 correction revision。原始网页分段及总时长不变，V1 查询、V2 下发和本机配额投影读取同一有效归属；新增或过期更正会触发终端刷新。
- **本周修正结果**：T.xia 2026-09-14 的 240 条、11,853 秒 `cg.163.com` 原始分段保持不变，有效归属已全部更正为 Restricted/Rest；不补时、不删时、不改网页 ACTIVE 或结算边界。
- **更正接口生产边界**：单批最多 99 个 segment，连同批次头严格保持在 D1 `batch()` 的 100 条语句边界内；避免名义 100 条请求在生产形成 101 条语句并整体失败。
- **自主度配置**：Pages 增加可编辑 Tab，本地 Admin 增加同名只读 Tab；可选择受限内容进入前是否完整确认，以及 Rest 软提醒无人响应时结束休息或默认继续。自主度只调整允许访问时的干预方式，不能绕过黑名单、时间窗及每日/每周/单站点硬配额。
- **内部托管发布**：功能提交 `c2841a3`、Guardian Worker `09c2b1c4-b4d4-4de8-8e05-d51f36cdd606`、控制台 Pages `7455c3ad` 与更新站点 `0a3707ac` 已发布；managed CRX 为 418,237 bytes，SHA256 `12d8e5417a34a6ec16bcf499dd55298827436adc17ba7e2e8cbf9d7cc994aa9f`，稳定扩展 ID 不变。终端升级和已知 P0 风险继续作为生产观察，不进入 Chrome Web Store。

---

## [1.7.31] — 2026-09-13

- **V2 配额直接接管**：新增统一 `quota_read_model_v2`，以本机完整账加云端其他设备已发布账计算日/周/单站点配额，并按设备 ID 排除云端本机副本。
- **取消云端锁**：V2 不再读取或保存云端配额锁；配额路由、定时检查、Rest 软提醒、Popup 和本地 Admin 从同一账本读模型即时计算，提高或取消限额后立即生效。
- **分设备完整性**：V2 快照增加 active domain 投影和 expected/missing/incompatible/stale/incomplete 设备状态；migration 027 仅记录 capability、扩展版本和最近上报时间。
- **明确降级**：无可信云端快照时仅使用本机账并标记其他设备未知；本机账不可读且无缓存时不伪报耗尽，受限娱乐暂时阻止。
- **显示口径**：本地 Admin 展示实际执行总账及来源；Pages 显示云端已确认账和设备贡献，不把终端未上传数据包装为云端实时总账。
- **记账冻结**：本版本不修改网页 ACTIVE、原始分段、本地结算、设备单账生成、历史账本或媒体配额隔离规则。
- **内部托管发布**：migration 027、Guardian Worker `0af8c265-d3c7-45c3-b10d-1870a0d648f9`、控制台 Pages `7d32c790` 与更新站点 `e9e4b8b4` 已发布；managed CRX 为 415,970 bytes，SHA256 `71cb832ba691660c3a1d833bdeb5f9f64115e9887e32b09889e9500367d41864`，稳定扩展 ID 不变。T.xia/P.xia 已切换 `timeQuota.accountingVersion=2`；设备升级和首批 V2 单账发布继续作为生产观察项。

---

## [1.7.30] — 2026-09-12

- **诊断证据可用性**：基础 warning/error 与关键健康摘要可持续上传，临时 info 支持 1/3/7/30 天；增加指定终端、最多 7 个自然日的一次性脱敏配额取证及完整批次校验。
- **网页原始事实传输修正**：统一零秒诊断事实协议；网页分段按 `id + contentHash` 逐项确认并拒绝同 ID 异内容；pending 独立于历史水位扫描，聚合 ACK 绑定本地 revision。
- **记账 V2 影子链路**：新增不可变设备日单账、最多 200 行分块、档案日/周原子发布和设备版本向量；正常合账只使用完整设备单账，不从原始事实重算。
- **独立对账与下发**：云端按固定原始事实截止点生成独立审计结果，mismatch 只报警不改账；周账以不可变分页快照下发，终端完整校验后仅写影子缓存。
- **产品隔离**：V2 不接入 Pages、Admin、Popup、配额、拦截或 V1 统计读取，不修改网页 ACTIVE、原始分段生成和本地统计秒数；正式切换仍需连续 7 日影子守恒、全设备兼容和下一个北京时间周一 00:00 门禁。
- **内部托管发布**：源码提交 `cf9f2c8`、D1 隔离 migration 023-026、Guardian Worker `e9e53258-f4d0-4c0a-bbe3-2b7a2b25bd27`、控制台 Pages `02e3bc0d` 与更新站点 `2bd564f4` 已发布；managed CRX 为 410,948 bytes，SHA256 `46662d97661a63f9f15d9641c78ef4c215b68b5f5499f870fa67e00c3eb99c2a`，稳定扩展 ID 不变。
- **风险保留**：19009 秒历史差额、`cg.163.com idleStateChanged` 少记和正式设备影子数据收敛均保持未解决或生产观察；发布影子能力不等于这些问题已经关闭。
...
- **统计收敛门禁**：当日原始 segment 未全部确认时不继续上传日/小时/目标物化；历史水位继续以远端完整性核对为推进条件。
