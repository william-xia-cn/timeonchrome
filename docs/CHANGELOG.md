...
- **Current source permission drift from submitted package**: current unpacked source keeps `scripting` permission for in-page mode notice content-script fallback. Before packaging or CWS resubmission, rebuild release evidence from the current `extension/` source and either justify this permission or remove the fallback and permission together.
...
- **Global PiP disable policy**: current policy is `disallow_all`. Any detected `isPiP=true` media fact, open `pip` media session, media checkpoint, or mode boundary attempts shared `EXIT_PIP` cleanup. Successful cleanup writes `pip_forbidden_cleanup`; failed cleanup preserves the factual `pip` session and records `pip_forbidden_cleanup_failed` diagnostics.
...
- **Segment Open/Close diagnostics schema gap**: local `usage_segments_v1` can include `description` for Open/Close operation diagnostics, and local admin may display those fields. Cloud `usage_segments_v1` ingestion still uses the fixed v1 upload schema and does not save `description`; before formal release, decide whether to keep this local-only or update upload payload, Worker validation/insert, D1 migration, cloud query, and Pages display.
...
- **Timing pipeline decoupling**: `background.js` is now documented as Chrome listener wiring only. Timing work is split into `core/timing-dispatcher.js`, `core/foreground-timing.js`, `core/media-timing.js`, and `core/checkpoint-scheduler.js`; foreground usage segments and local media segments are separate ledgers, and checkpoint runners are independently traced/fail-isolated. Cloud upload, Worker, D1, and Pages schemas are unchanged.
