...
- **Local media ledger schema gap**: media timing now uses local-only `media_facts_v1` / `media_sessions_v2` / `media_segments_v1` / `daily_media_stats_v1`, with classes `foregroundAudio`, `backgroundAudio`, `foregroundVideo`, `backgroundVideo`, and `pip`. Cloud upload, Worker validation, D1 schema, cloud query, and Pages display do not yet ingest these media segments. Before formal release, decide whether media细分 remains local-only or gets a dedicated cloud schema/update path.
- **Foreground media webpage compensation**: an unminimized Chrome active tab playing media can keep ordinary webpage `ACTIVE` even when Chrome is unfocused or the system is idle; `locked` still blocks ordinary foreground webpage time. This is now the unified media webpage timing rule and should be covered in release verification.
